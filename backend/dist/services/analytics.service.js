...
exports.getPareto = getPareto;
exports.getConsumptionVsHours = getConsumptionVsHours;
exports.getHealthScore = getHealthScore;
const prisma_1 = require("../database/prisma");
async function getPareto() {
    const grouped = await prisma_1.prisma.intervention.groupBy({
        by: ["equipementId"],
        _sum: { quantite: true },
    ...
    return Promise.all(grouped.map(async (item) => {
        ...
        cumulative += value;
        const eq = await prisma_1.prisma.equipement.findUnique({ where: { id: item.equipementId } });
        return {
...
async function getConsumptionVsHours() {
